...
public class RealTimeChartEMem extends ChartPanel {
	private static Logger logger = Logger.getLogger(RealTimeChartEMem.class);
	...
	public void addSery(float ratio) {
		...
		logger.debug("EMem:      mili = " + mili + "           ratio = " + ratio);
	}

	public RealTimeChartEMem(String yaxisName, Double timeLen) {
	...
	private static JFreeChart createChart(String yaxisName, Double timeLen) {
		// 创建时序图对象
		timeSeries = new TimeSeries("memory ratio", Millisecond.class);
		TimeSeriesCollection timeseriescollection = new TimeSeriesCollection(timeSeries);
		JFreeChart jfreechart = ChartFactory.createTimeSeriesChart("Memory", null, yaxisName, timeseriescollection, true, true, false);
